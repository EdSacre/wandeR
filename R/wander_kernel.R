...
#'     can be implemented in the 'connect' function. There are two dispersal kernel options,
#'     a negative exponential (default) or a beta distribution. Test different parameter values
#'     to inform what values are chosen for the connectivity modelling.
#'
#' @param maxdist The maximum dispersal distance of the species (units are not relevant). Default is 10,000.
#' @param kernel The dispersal kernel distribution. Options are 'neg_exp' or 'beta'. Default is 'neg_exp'.
...
wander_kernel <- function(maxdist = 10000, kernel = c("neg_exp", "beta"), t = NULL, a = NULL, b = NULL){
  ...
  if(kernel == "neg_exp" & !is.null(a) | !is.null(b)){
    warning("A negative exponential distribution was specified, as well as 'a' and 'b' parameters. Did you mean to choose a beta distribution?")
  }
  if(kernel == "beta" & !is.null(t)){
    warning("A beta distribution was specified, as well as the 't' parameter. Did you mean to choose a negative exponential distribution?")
  }
